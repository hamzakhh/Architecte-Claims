...
@Component({
  selector: 'app-tous-sinistres',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './toussinistres.html'
})
export class TousSinistres implements OnInit {
  ...
  // Prendre en charge
  prendreEnCharge(claimId: string): void {
    this.claimService.prendreEnCharge(claimId).subscribe({
      next: () => {
        this.success.set('Sinistre pris en charge');
        this.loadData();
        this.clearSuccessAfterDelay();
      },
      error: () => this.error.set('Erreur lors de la prise en charge')
    });
  }
  ...
  validerExpertise(expertiseId: string): void {
    this.expertiseService.validerExpertise(expertiseId).subscribe({
      next: () => {
        this.success.set('Expertise validée');
